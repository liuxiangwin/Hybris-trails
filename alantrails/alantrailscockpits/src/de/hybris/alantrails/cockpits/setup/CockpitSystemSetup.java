 ...
 *  
 */
package de.hybris.alantrails.cockpits.setup;
...
import de.hybris.alantrails.cockpits.constants.AlantrailsCockpitsConstants;
 ...
 */
@SystemSetup(extension = AlantrailsCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(AlantrailsCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(AlantrailsCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						AlantrailsCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/alantrailscockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
